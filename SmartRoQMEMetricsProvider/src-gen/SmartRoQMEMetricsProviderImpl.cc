//--------------------------------------------------------------------------
// Code generated by the SmartSoft MDSD Toolchain
// The SmartSoft Toolchain has been developed by:
//  
// Christian Schlegel (schlegel@hs-ulm.de)
// University of Applied Sciences Ulm
// Prittwitzstr. 10
// 89075 Ulm (Germany)
//
// Information about the SmartSoft MDSD Toolchain is available at:
// www.servicerobotik-ulm.de
//
// Please do not modify this file. It will be re-generated
// running the code generator.
//--------------------------------------------------------------------------
#include "SmartRoQMEMetricsProviderImpl.hh"

SmartRoQMEMetricsProviderImpl::SmartRoQMEMetricsProviderImpl(const std::string &componentName, int & argc, char ** argv)
:	CHS::SmartComponent(componentName, argc, argv)
{  }

SmartRoQMEMetricsProviderImpl::~SmartRoQMEMetricsProviderImpl() 
{  }

CHS::StatusCode SmartRoQMEMetricsProviderImpl::run(void)
{
	   CHS::StatusCode result = CHS::SMART_OK;

	   // open thread for managing all server-initiated-disconnects in this component
	   srvInitDiscHandler.open();

	   // if meanwile strg+c was called -> return immediatelly, without waiting on substate or reactor
	   if(!first_call_of_handle_signal) return CHS::SMART_ERROR;

	   componentIsRunning = true;

	   ///////////////////////////////////////////////////////////
	   //                                                       //
	   //  component is now fully initialized and running.      //
	   //  (untill the component is commanded to shutdown...)   //
	   //                                                       //
	   ///////////////////////////////////////////////////////////
	   if(getStateSlave() != NULL)
	   {
	      // block this thread till component is commanded to shut down...
	      getStateSlave()->acquire("shutdown");
	   } else {
	     // block this thread until component is commaned to shut down...
	     this->waitOnRuntimeCondVar();
	   }

	   return result;
}
void SmartRoQMEMetricsProviderImpl::closeAllAssociatedTasks(const int &taskShutdownTimeLimit)
{
	if(this->getStateSlave() != NULL) {
		getStateSlave()->release("shutdown");
	}
	this->signalSmartTasksToStop(ACE_Time_Value(taskShutdownTimeLimit));
}
void SmartRoQMEMetricsProviderImpl::cleanUpComponentResources()
{
	this->cleanUpInternalResources();
}
